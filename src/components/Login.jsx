...
import CredentialChangeModal from "./CredentialChangeModal";
...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Check if credential change is required (only for pre-registered employees)
      ...
      setTimeout(() => {
        if (result.user.userLevel === "admin") {
          ...
          result.user.userLevel === "nurse" ||
          result.user.userLevel === "staff"
        ) {
  ...
  const handleCredentialUpdateSuccess = () => {
    ...
    setNotification({
      message:
        "Registration completed successfully! Please login with your new credentials.",
      type: "success",
    });
    ...
    // The user needs to login again with their new credentials
  ...
  const handleCredentialModalClose = () => {
    setShowCredentialModal(false);
    // When user closes the modal without completing registration, log them out
    localStorage.removeItem("token");
    localStorage.removeItem("user");
    setNotification({
      message: "Please complete your registration to access the system",
      type: "warning",
    });
  };
  ...
  return (
    ...
    >
        ...
        {notification.message && (
          <motion.div
            ...
            style={{
              ...
              padding: "1rem",
              backgroundColor:
                theme.notification[notification.type]?.bg ||
                theme.notification.info.bg,
              color:
                theme.notification[notification.type]?.text ||
                theme.notification.info.text,
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Modern dialysis care simplified with intuitive technology that
              enhances patient care and clinical workflows.
            </motion.p>
            {windowSize.width > 768 && (
              <motion.div
                style={{
                ...
                <div style={{ marginBottom: "1.5rem" }}>
                  ...
                  <motion.div
                    style={{
                      ...
                      borderRadius: "0.75rem",
                      border: `1px solid ${
                        errors.password ? "#f87171" : theme.border
                      }`,
                      backgroundColor: theme.inputBg,
                      backdropFilter: "blur(5px)",
                      WebkitBackdropFilter: "blur(5px)",
      ...
      {/* Credential Change Modal - Only shown after successful login with pre-registered status */}
      {showCredentialModal && (
